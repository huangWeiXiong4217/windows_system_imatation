...
public class File implements Serializable {
	...
	public File(String fileName) {
		...
		setFileNameP();
    if (this.hasParent()) {
      Folder parent1 = (Folder) this.getParent();
	...
	public File(String fileName, String location, int diskNum, Folder parent) {
		...
		this.size = this.getSize(content.length());
		this.space = size + "B";
  ...
  public static double getSize(int length) {
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
    	...
    	flagP = new SimpleStringProperty(flag == 0 ? "只读" : "读写");
